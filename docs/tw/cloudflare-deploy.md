...
本專案可以通过 **OpenNext 适配器** 部署为 **Cloudflare Worker**，为您提供：

- 全球边缘部署
- 极低延迟
- 免费的 `workers.dev` 域名托管
- 通过 R2 实现完整的 Next.js ISR 支援（可选）

> **Windows 使用者重要提示：** OpenNext 和 Wrangler 在 **原生 Windows 環境下并不完全可靠**。建议方案：
>
> - 使用 **GitHub Codespaces**（完美執行）
> - 或者使用 **WSL (Linux)**
>
> 纯 Windows 构建可能会因为 WASM 檔案路径問題而失敗。
...
Cloudflare 在本地測試时使用不同的檔案。

### 1) 建立 `.dev.vars`（用于 Cloudflare 本地除錯 + 部署）
...
### 選項 A：不使用 R2 部署（简单，免费）

如果您不需要 ISR 缓存，可以選擇不使用 R2 进行部署：
...
### 選項 B：使用 R2 部署（完整的 ISR 支援）

R2 开启了 **增量静态再生 (ISR)** 缓存功能。需要在您的 Cloudflare 账户中绑定支付方式。

**1. 在 Cloudflare 控制台中建立 R2 存储桶：**
...
> **重要提示：** `bucket_name` 必须与您在 Cloudflare 控制台中建立的名称完全一致。
...
**選項 1：通过 Cloudflare 控制台（推荐）**

存取：https://dash.cloudflare.com → Workers & Pages → Overview → Set up a subdomain

**選項 2：在部署过程中**

執行 `npm run deploy` 时，Wrangler 可能会提示：
...
> **注意：** 在 CI/CD 或非交互式環境中，该提示不会出现。请先通过控制台进行注册。
...
**修复：** 启用 R2（需要支付方式）或使用選項 A（不使用 R2 部署）。
...
**修复：** 添加 `r2_buckets` 部分或切换到選項 A（不使用 R2）。

---

### `Can't set compatibility date in the future`

**原因：** wrangler 配置中的 `compatibility_date` 設定为了未来的日期。
...
**原因：** Windows 檔案名不能包含 `?`，但某个 wasm 资源檔案名中使用了 `?module`。

**修复：** 在 Linux 環境（WSL、Codespaces 或 CI）上进行构建/部署。
...
測試或简单應用请選擇 **不使用 R2**。需要 ISR 缓存的生产環境應用请選擇 **使用 R2**。
