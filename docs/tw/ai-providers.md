...
1. 将 `.env.example` 複製为 `.env.local`
2. 設定所选提供商的 API 密钥
3. 将 `AI_MODEL` 設定为所需的模型
4. 執行 `npm run dev`
...
AI_MODEL=deepseek-ai/DeepSeek-V3  # 範例；使用任何 SiliconFlow 模型 ID
```
...
注意：在 AWS 環境（Lambda、带有 IAM 角色的 EC2）中，凭证会自动从 IAM 角色获取。
...
Vercel AI Gateway 通过单个 API 密钥提供对多个 AI 提供商的统一存取。这简化了身份驗證，让您无需管理多个 API 密钥即可在不同提供商之间切换。
...
-   如果未設定 `AI_GATEWAY_BASE_URL`，则使用預設的 Vercel Gateway URL (`https://ai-gateway.vercel.sh/v1/ai`)
-   自定义基础 URL 适用于：
    -   使用自定义网关实例进行本地开发
    -   自托管 AI Gateway 部署
    -   企业代理配置
...
如果您只配置了**一个**提供商的 API 密钥，系統将自动检测并使用该提供商。无需設定 `AI_PROVIDER`。

如果您配置了**多个** API 密钥，则必须显式設定 `AI_PROVIDER`：
...
管理员可以配置多个服务端模型，让所有使用者无需提供个人 API Key 即可使用。

### 配置方式

**方式一：環境變數**（推荐用于云部署）

設定 `AI_MODELS_CONFIG` 为 JSON 字符串：
...
在專案根目录建立 `ai-models.json` 檔案（或通过 `AI_MODELS_CONFIG_PATH` 指定路径）。
...
| 字段 | 必填 | 說明 |
|------|------|------|
| `name` | 是 | 显示名称（支援同一提供商多个配置） |
| `provider` | 是 | 提供商类型（`openai`, `anthropic`, `google`, `bedrock` 等） |
| `models` | 是 | 模型 ID 列表 |
| `default` | 否 | 设为 `true` 表示預設选中该提供商的第一个模型 |
| `apiKeyEnv` | 否 | 自定义 API Key 環境變數名（預設使用提供商标准變數如 `OPENAI_API_KEY`） |
| `baseUrlEnv` | 否 | 自定义 Base URL 環境變數名 |

### 說明

- API Key 和凭证通过環境變數提供。預設使用标准變數名（如 `OPENAI_API_KEY`），也可通过 `apiKeyEnv` 指定自定义變數名。
- `name` 字段允许同一提供商多个配置（例如 "OpenAI Production" 和 "OpenAI Staging" 都使用 `provider: "openai"` 但 `apiKeyEnv` 不同）。
- 如果配置不存在，應用会回退到 `AI_PROVIDER`/`AI_MODEL` 環境變數配置。

## 模型能力要求

此任务对模型能力要求极高，因为它涉及產生具有严格格式约束（draw.io XML）的长文本。

**推荐模型**：

-   Claude Sonnet 4.5 / Opus 4.5

**关于 Ollama 的說明**：虽然支援将 Ollama 作为提供商，但除非您在本地執行像 DeepSeek R1 或 Qwen3-235B 这样的高效能模型，否则对于此用例通常不太实用。
...
```bash
TEMPERATURE=0  # 輸出更具确定性（推荐用于图表）
```

**重要提示**：对于不支援温度設定的模型（例如以下模型），请勿設定 `TEMPERATURE`：
- GPT-5.1 和其他推理模型
- 某些专用模型
...
-   **最佳体验**：使用支援视觉的模型（GPT-4o, Claude, Gemini）以获得图像转图表功能
-   **经济实惠**：DeepSeek 提供具有竞争力的价格
-   **隐私保护**：使用 Ollama 进行完全本地、离线的操作（需要强大的硬件支援）
-   **灵活性**：OpenRouter 通过单一 API 提供对众多模型的存取
